defmodule Funx.Optics.Prism do
  ...
  **Important:** `review` constructs a fresh structure from the focused value alone—it does not merge
  ...
  @doc """
  Builds a prism that filters by struct type, returning `Just(true)` on match.

  This is similar to `struct/1`, but instead of returning the entire struct on match,
  it returns `Just(true)`. This makes it ideal for type partitioning and filtering in
  sorting contexts where you don't want the prism to impose struct-field ordering.

  **Use this instead of `struct/1` when:**
  - You want to partition/filter by type in sorting (e.g., `desc matches_struct(Check)`)
  - You only care about type membership, not the struct's internal values
  - You want to avoid struct-field comparison interfering with subsequent sort criteria

  **Use `struct/1` when:**
  - You need to extract or modify the struct value
  - You're composing with other prisms to access fields
  - You want struct-field ordering to matter

  ## Examples

      # Filtering by type in ord DSL - the primary use case
      use Funx.Ord.Dsl

      ord do
        desc Prism.matches_struct(Check)    # Checks first, no struct comparison
        asc Prism.key(:routing_number)      # Then sort by routing_number
        asc Lens.key(:name)                 # Then by name
      end

      # Preview behavior comparison
      check = %Check{name: "Alice", routing_number: "12345"}

      # struct/1 returns the full struct
      Prism.preview(check, Prism.struct(Check))
      #=> %Just{value: %Check{name: "Alice", routing_number: "12345"}}

      # matches_struct/1 returns true for matches
      Prism.preview(check, Prism.matches_struct(Check))
      #=> %Just{value: true}

      # Both return Nothing for non-matches
      Prism.preview(%CreditCard{}, Prism.matches_struct(Check))
      #=> %Nothing{}

  ## Why This Matters for Sorting

  When using `desc Prism.struct(Check)` in an ord DSL, the prism returns the entire
  Check struct for matches. With `Maybe.lift_ord`, this causes Elixir to compare the
  full structs field-by-field (`:account_number`, `:amount`, `:name`, `:routing_number`),
  which interferes with your intended sort criteria.

  Using `matches_struct/1` instead returns `true` for all matches, so all Checks compare
  equal on this criterion, allowing subsequent criteria to determine the order.
  """
  @spec matches_struct(module()) :: t(struct(), boolean())
  def matches_struct(mod) when is_atom(mod) do
    unless function_exported?(mod, :__struct__, 0) do
      raise ArgumentError,
            "#{inspect(mod)} is not a struct module. " <>
              "Prism.matches_struct/1 requires a module with defstruct."
    end
