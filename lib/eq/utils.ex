defmodule Monex.Eq.Utils do
  ...
  def eq_by?(f, a, b, eq \\ Eq) do
    case eq do
      module when is_atom(module) -> module.eq?(f.(a), f.(b))
      %{} = custom_map -> custom_map[:eq?].(f.(a), f.(b))
    end
  end
  ...
  """
  def equal?(a, b, eq \\ Eq) do
    case eq do
      module when is_atom(module) -> module.eq?(a, b)
      %{} = custom_map -> custom_map[:eq?].(a, b)
    end
  end
