defmodule Funx.Monad.Maybe.Dsl.TransformerTest do
  @moduledoc """
  Tests for the Maybe DSL Transformer behaviour and built-in transformers.
  """
  ...
  describe "Transformer.apply_transformers/3" do
    ...
    test "stops on first error" do
      ...
      defmodule NeverRunTransformer do
        ...
        def transform(steps, _opts),
          do: {:ok, steps ++ [%Step.Map{operation: String, opts: []}]}
      end
  ...
  describe "Integration with DSL" do
    ...
    test "applies transformer during compilation" do
      result =
        maybe 42, transformers: [TestTransformer], as: nil do
          map &(&1 * 2)
        end

      # Should have applied both the explicit map and the transformer's map
      assert {:transformed, 84} = result
    end
    ...
    test "raises CompileError when transformers is not a list" do
      assert_raise CompileError, ~r/Invalid transformers.*Must be a list of modules/, fn ->
        Code.eval_quoted(
          quote do
            require Funx.Monad.Maybe.Dsl
            import Funx.Monad.Maybe.Dsl

            maybe 42, transformers: :not_a_list do
              map &(&1 * 2)
            end
          end
        )
      ...
      assert_raise CompileError, "custom transformation error", fn ->
        Code.eval_quoted(
          quote do
            require Funx.Monad.Maybe.Dsl
            import Funx.Monad.Maybe.Dsl
            alias Funx.Monad.Maybe.Dsl.TransformerTest.ErrorTransformerString

            maybe 42, transformers: [ErrorTransformerString] do
              map &(&1 * 2)
            end
          end
        )
      ...
      assert_raise ArgumentError, "custom exception error", fn ->
        Code.eval_quoted(
          quote do
            require Funx.Monad.Maybe.Dsl
            import Funx.Monad.Maybe.Dsl
            alias Funx.Monad.Maybe.Dsl.TransformerTest.ErrorTransformerException

            maybe 42, transformers: [ErrorTransformerException] do
              map &(&1 * 2)
            end
          end
        )
