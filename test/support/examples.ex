defmodule Funx.Monad.Either.Dsl.Examples do
  ...
  These modules implement `run/2` from the `Funx.Monad.Dsl.Behaviour`.
  ...
  defmodule ParseInt do
    ...
    Implements `run/2` which can be used with `bind` or `map`.
    """
  ...
  defmodule Double do
    ...
    Implements `run/2` to be used with `map`.
    """
    @behaviour Funx.Monad.Dsl.Behaviour
  ...
  defmodule FileReader do
    ...
    Demonstrates tuple support - `run/2` returns `{:ok, content}` or
  ...
  defmodule JsonParser do
    ...
    Demonstrates tuple support - `run/2` returns `{:ok, decoded}` or
  ...
  defmodule TupleParseInt do
    ...
    Demonstrates tuple support - `run/2` returns `{:ok, int}` or
    `{:error, reason}` which is automatically converted to Either.
    """
    @behaviour Funx.Monad.Dsl.Behaviour
  ...
  defmodule TupleValidator do
    ...
    Demonstrates tuple support for validation - `run/2` returns `{:ok, value}` or
    `{:error, reason}` which is automatically converted to Either.
    """
    @behaviour Funx.Monad.Dsl.Behaviour
  ...
  defmodule InvalidReturn do
    ...
    This module demonstrates what happens when `run/2` returns an invalid type.
